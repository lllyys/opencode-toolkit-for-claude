...
If `{config_skip_patterns}` is set (from `.opencode-toolkit.md`), filter the file list:

1. For each file in scope, check against every skip pattern (glob matching)
2. Remove any file that matches a skip pattern
3. If ALL files are filtered out, respond: "All files in scope are excluded by skip patterns in `.opencode-toolkit.md`. Nothing to audit." and STOP.
